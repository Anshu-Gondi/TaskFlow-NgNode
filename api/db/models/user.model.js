...
UserSchema.methods.generateTokenId = function () {
    const user = this;
    return new Promise((resolve, reject) => {
        // Create a token ID using the user's _id and the custom access level "tokenId"
        jwt.sign(
            { _id: user._id.toHexString(), access: 'tokenId' },
            jwtSecret, // Use the jwtSecret constant from your environment or code
